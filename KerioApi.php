...
require_once(dirname(__FILE__) . '/../../examples/class/HtmlHelper.php');
...
class KerioApi implements KerioApiInterface {
	...
	public function sendRequestRaw() {
			
		//{"jsonrpc":"2.0","id":1,"method":"Batch.run","params":{"commandList":[{"method":"IpAddressGroups.apply"},{"method":"Session.getConfigTimestamp"}]}}
		
		$rawreq = '{"jsonrpc":"2.0","id":'.$this->getRequestId().',"token":"'.$this->token.'","method":"Batch.run","params":{"commandList":[{"method":"IpAddressGroups.apply"},{"method":"Session.getConfigTimestamp"}]}}';

		/* Send data to server */
		$json_response = $this->send('POST', $rawreq);
